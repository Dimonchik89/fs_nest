...
export class FilesService {
	...
	async createFolder(userPath): Promise<string> {
		const folderPath = join(path, process.env.UPLOADS_BASE_PATH, userPath);
		await fsExtra.ensureDir(folderPath);
		return folderPath;
	}

	async uploadArrayFiles({
		files,
		reqUser,
	}: {
			...
			if (existingFileByOriginalName) {
				try {
					...
					if (unlinkError.code !== 'ENOENT') {
						console.warn(
							`Could not delete old file ${fullFilePath}:`,
							unlinkError,
						);
					}
				}
	...
	async uploadFiles(
		...
		const incomingFilesTotalSize = files.reduce(
